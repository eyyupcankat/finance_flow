  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between">
        ...
        <span className={`text-xs font-medium px-2 py-1 rounded-full flex items-center gap-1 ${trendPositive ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-950' : 'text-red-500 bg-red-50 dark:bg-red-950'}`}>
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-5 relative">
      {feedback && (
        <div className={`fixed top-20 right-6 z-[100] px-6 py-3 rounded-xl shadow-lg border animate-in slide-in-from-right-full ${feedback.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'
          }`}>
          {feedback.message}
        </div>
      )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 transition font-mono tracking-wider"
                  />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Bank Name</label>
                <div className="relative">
                  <Building2 size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                  <select
                    value={bank}
                    onChange={(e) => setBank(e.target.value)}
                    className="w-full pl-9 pr-8 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 appearance-none transition"
                  >
      ...
      <div className="grid grid-cols-5 gap-5">
        <div className="col-span-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Recent Transactions</h3>
          ...
          {transactions.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No transactions found. Connect a card first.</p>
          ) : (
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {transactions.slice(0, 5).map((tx, i) => (
                  <tr key={i} className="text-sm">
                    <td className="py-3 text-gray-400 dark:text-gray-500 text-xs">{tx.transactionDate}</td>
                    <td className="py-3 font-medium text-gray-800 dark:text-gray-100">{tx.merchant}</td>
                    <td className="py-3">
        ...
        <div className="col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 flex flex-col">
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Detected Subscriptions</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Based on monthly recurring patterns</p>
          </div>
          <div className="space-y-3 flex-1">
            {subscriptions.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No active subscriptions detected.</p>
            ) : (
          ...
          <Link
            to="/subscriptions"
            className="mt-4 w-full text-center py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
      ...
      {showAllTransactions && (
        ...
        >
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-3xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]" onMouseDown={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800">
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">All Transactions</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Showing {transactions.length} total transactions</p>
              </div>
              <button onClick={() => setShowAllTransactions(false)} className="p-1.5 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition">
                <X size={18} />
              </button>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {paginatedTransactions.map((tx, i) => (
                    <tr key={i} className="text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <td className="py-4 text-gray-400 dark:text-gray-500 text-xs">{tx.transactionDate}</td>
                      <td className="py-4 font-medium text-gray-800 dark:text-gray-100">{tx.merchant}</td>
                      <td className="py-4">
            ...
            {totalPages > 1 && (
              <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50 dark:bg-gray-800">
                <button
                  onClick={() => setTxPage(p => Math.max(1, p - 1))}
                  disabled={txPage === 1}
                  className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg disabled:opacity-50 transition"
                >
                  <ChevronLeft size={16} /> Previous
                </button>
                <span className="text-sm font-medium text-gray-600 dark:text-gray-300">Page {txPage} of {totalPages}</span>
                <button
