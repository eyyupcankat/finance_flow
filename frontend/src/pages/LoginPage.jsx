...
export default function LoginPage() {
  ...
  const handleVerifyMfa = async (e) => {
    ...
    setError('')
    try {
      console.log('Verifying MFA code:', mfaCode, 'for user:', tempUserId)
      ...
      console.log('MFA verification successful, user data:', userData)
      login(userData, res.data.token)
      navigate('/dashboard')
    } catch (err) {
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
        ...
        {mfaRequired ? (
          <form onSubmit={handleVerifyMfa} className="space-y-4">
            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3 mb-2">
              <Shield size={24} className="text-emerald-500" />
              <div>
                <p className="text-sm font-semibold text-emerald-700">MFA Required</p>
                <p className="text-xs text-emerald-600">Enter the verification code sent to your device.</p>
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  autoFocus
                  className="w-full px-4 py-2.5 text-center text-xl font-bold tracking-widest border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 transition"
                />
              </div>
              <p className="text-[10px] text-gray-400 mt-2 text-center">
                For demo purposes, use code: <span className="font-mono font-bold text-gray-600">123456</span>
              </p>
            </div>
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2.5 rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
            >
